...
export default async function translate(client: CommandClient) {
	...
	assert("yandex_translation_key" in translationKeys, `expected 'yandex_translation_key' in 'config/translate.json'`)
	...
	client.defineCommand({
		...
		args: [{
			...
			type: "lang",
			fail_message: `The 'translation_direction' argument accepts ${client.types.get('lang')!.display}`,
			required: true
		}],
