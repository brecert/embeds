...
export default function (client: CommandClient) {
  ...
  client.defineCommand({
    ...
    async run([tags, booru]: [string, string]) {
      const [img] = await Booru.search(booru || "safebooru", tags, {
        limit: 1,
        random: true,
      });
      ...
      if (img === undefined) {
        return {
          title: "ERROR",
          description:
            `there was a problem getting the image with the tags "${tags}"`,
        };
      ...
      return {
        name: `👍${img.score} ${
          (img as any).down_score !== undefined
            ? `👎${(img as any).down_score}`
            : ""
        } ${
          (img as any).fav_count !== undefined
            ? `⭐${(img as any).fav_count}`
            : ""
