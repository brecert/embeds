...
interface IGenderizeResult { name: string, gender: "male" | "female" | null, probability: number, count: number }
	...
	client.defineCommand({
	  ...
	  async run([name]: [string]) {
	    ...
	    if(stats.gender === null) {
	    	return {
	    		title: stats.name,
	    		description: `No one is registered under the name '${name}' in our current samples`
	    	}
	    }
	      ...
	      // description: `${stats.probability * 100}% of ${stats.gender}s have the name ${stats.name} in a sample count of ${stats.count}`
    		description: `In a sample of ${stats.count} people named ${stats.name}, ${stats.probability * 100}% of them are ${stats.gender}`
    	}
	  }
	})
}